...
func (p *CoverageProcessor) processRustCoverage(ctx context.Context, opts ProcessOptions) error {
	...
	if binaryPath == "" {
		fmt.Println("   Warning: No instrumented binary found. Profdata is available but reports cannot be generated.")
		fmt.Printf("   To generate reports manually, run:\n")
		fmt.Printf("     %s export --format=lcov --instr-profile=%s <path-to-binary>\n", llvmCov, profdataPath)
		return nil
	}
	fmt.Printf("   Binary: %s\n", binaryPath)
	...
	if err != nil {
		// Try without filters in case they caused issues
...
func findLLVMTool(name string) (string, error) {
	...
	if rustcPath, err := exec.LookPath("rustc"); err == nil {
		...
		if err == nil {
			...
			candidates := []string{
				filepath.Join(sysroot, "lib", "rustlib", runtime_target(), "bin", name),
...
func runtime_target() string {
	return fmt.Sprintf("%s-%s", runtime_arch(), runtime_os())
}

func runtime_arch() string {
